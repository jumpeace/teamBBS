        ...
        if (update_name.val() !== before_name) {
            $.ajax({
                ...
                .done((response, textStatus, jqXHR) => {
                    ...
                    $('#channel_name').text(objectResponse.name);
                    update_name.val(objectResponse.name)
                    console.log(`Updated channel name. ${jqXHR.status}`)
                })
                .fail(jqXHR => {
                    alert(`チャンネル名を変更できませんでした（Status Code: ${jqXHR.status}）`);
                });
        }
